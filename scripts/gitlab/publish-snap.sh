...
SNAP_PACKAGE="openetc_"$VERSION"_"$BUILD_ARCH".snap"
# Choose snap release channel based on OpenETC version track
case ${TRACK} in
...
echo $SNAPCRAFT_LOGIN_OPENETC_BASE64 | base64 --decode > snapcraft.login
snapcraft login --with snapcraft.login
snapcraft push --release $CHANNEL $SNAP_PACKAGE
snapcraft status openetc
snapcraft logout
